...
function restartQuiz(){
    ...
    document.querySelector(".answerSection").remove();
    addListener();
}
...
function createDiv(){
    ...
    document.addEventListener('click',function(e){  
        if(e.target && (e.target.id=="answerButton" || e.target.id=="answerP")){
            restartQuiz();
        }
    })
        ...
        if(!isSelected(e.currentTarget)){   //non selezionato
            for (const image of images)
            {
                if(isSelected(image) && getQuestionId(image)==getQuestionId(e.currentTarget)){  //cerco la vecchia scelta e la deseleziono
                    //ottimizabile effettuando controllo solo nel casogetQuestionId(image) 
                    uncheck(image);
                    break;
                }
            }
